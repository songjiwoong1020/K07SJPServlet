...
public class ChatServer02 {
	...
	/*
	-해당 set컬렉션은 클라이언트가 접속 할 때마다 세션ID를 저장하므로
	static으로 선언된다.
	-접속한 웹 브라우저가 웹소켓을 지원해야하며, 웹브라우저를 닫으면 Onclose가 호출된다.
		...
		synchronized (clients) {
			...
			for(Session client : clients) {
				...
				if(!client.equals(session)) {
					client.getBasicRemote().sendText(message);//나를 제외한 클라이언트에게 에코
				}
			}
		}
	}
