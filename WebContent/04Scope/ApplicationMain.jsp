...
- 어플리케이션 영역에 저장된 속성은 모두 JSP(Servlet) 페이지에서
	공유된다.
...
//List계열의 컬렉션에 DTO객체 저장(ArrayList와 Vector은 기본적 사용법은 동일함)
...
//Map계열의 컬렉션에 DTO객체 저장(key, value를 통해 저장)
...
<body>
	...
	<h2>어플리 케이션 영역의 공유</h2>
	
	<!-- 
		새로운 페이지에 대한 요청이므로 page영역, request영역 둘다
		공유되지 않는다.
	 -->
	<h3>사용자 클릭에 의한 이동</h3>
	...
	<!--
		외부파일 인클루드  : include 지시어를 통해 페이지를 삽입하면
			파일의 내용이 그대로 포함 된 후 컴파일되므로 하나의 페이지라
