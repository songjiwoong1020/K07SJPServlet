...
*/
// HTTP/1.0에서 적용 할 때 사용(현재는 사용하지 않지만 호환성을 위해 남겨둠)
response.setHeader("pragma", "no-cache");
// HTTP/1.1에서 적용 할 때 사용
...
파일 다운로드 구현시 사용
	...
	jpg와 같은 이미지 파일은 웹브라우저가 인섹하는 MIME타입이므로
	다운로드를 위해서는 아래와같은 설정이 필요하다.
*/
// response.setContentType("bibary/octect-stram");

/*
응답헤더 추가/설정하기
	add계열 : 기존의 헤더명이 존재 할 경우 기존의 값을 유지하면서
		계속 추가한다. 없을때에는 새롭게 생성한다.
	set계열 : 기존 헤더명이 존재 할 경우 기존값이 변경된다.
		없을때는 새롭게 생성한다.
*/
...
response.setIntHeader("numberOfCrew", 99);//기존 해더 덮어쓰기(수정) -> 99만 출력
%>
...
<body>
	...
	<%=response.containsHeader("Content-type") ? "존재함" : "존재하지 않음" %>
