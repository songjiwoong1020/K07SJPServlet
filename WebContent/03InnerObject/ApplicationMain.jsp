...
<body>
	...
	얻어 올 수 있다. 사용시 context root를 제외한 /로 시작하는 절대 경로를
	입력해야 한다.
	 -->
	<h2>서버 물리적 경로 얻어오기</h2>
	<ul>
		<li>application내장객체 : <%=application.getRealPath("/images") %>
		</li>
		<li>request내장객체 : <%=request.getServletContext().getRealPath("/images") %>
		</li>
		<li>request내장객체(주로 서블릿에서 사용) : <%=request.getRealPath("/images") %>
		</li>
		<li>session내장객체 : <%=session.getServletContext().getRealPath("/images") %>
		</li>
		<li>config내장객체 : <%=config.getServletContext().getRealPath("/images") %>
		</li>
		<li>this키워드(주로선언부에서사용) : <%=this.getServletContext().getRealPath("/images") %>
		</li>
	</ul>
	
	<!-- 
	선언부에서 JSP의 내장객체를 사용할때는 해당 내장객체의 클래스타입으로
	사용해야한다. application객체는 ServletContext타입을
	가지고있다.
	 -->
	<h2>선언부에서 물리적경로 사용하기</h2>
	<%!
	//1.this키워드 사용하기. -> this자체가 context
	String getRealPath(){
		return
