...
/*
	운영처제별로 서버의 물리적 경로는 달라 질 수 있으므로 파일이
	업로드 되는 정확한 위치를 알기 위해서 반드시 필요한 정보이다.
*/
String saveDirectory = application.getRealPath("/Upload");

//2.업로드 할 파일의 최대 용량 설정(바이트단위)
/*
...
//4.파일명 중복 처리
/*
	파일명이 중복되는 경우 자동인덱스를 부여햐여 파일명을 수정해준다.
	Ex) apple.png, apple1.png, apple2.png ... 이와 같은 형태로 이름을 부여한다.
*/
...
try{
	...
	만약 예외가 발생한다면 주로 최대 용량 초과 혹은 디렉토리 경로가
	잘못된 경우가 대부분이다.
	*/
...
<body>
	...
	<ul>
		...
		<li>파일크기 :
		<%=(int)Math.ceil(mr.getFile("chumFile1").length() / 1024.0) %> kb</li>
		<li>이미지표현 :
	...
	<ul>
		...
		<li>파일크기 :
		<%=(int)Math.ceil(mr.getFile("chumFile2").length() / 1024.0) %> kb</li>
		<li>이미지표현 :
