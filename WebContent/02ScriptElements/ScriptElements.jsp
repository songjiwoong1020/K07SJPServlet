<%--
	...
	지시자(Directive) : 페이지 속성을 지정하거나 외부분서를
		include할 때 사용한다.
		사용법 : <%@ 지시자키워드
 --%>
...
<%--
	선언부(Declaration) : 스크립트렛이나 표현식에서 사용 할
		메소드를 선언 할 때 사용한다. 함수는 주로 중복해서 사용하는
...
<body>
	...
	<%-- 
	스크립트렛(Scriptlet) : 주로 JSP코드를 작성하고 실행 할 때 사용하는
		영역이다. 스크랩트렛은 head영역, body영역 어디서든 사용 할 수 있다.
		사용법 : <% JSP코드;
	 --%>
	<%
	int value1 = 3;
	...
	<%--
	표현식(Expression) : 변수를 웹브라우저상에 출력 할 때 사용한다.
		JS의 docoment.write()와 동일한 역할을 한다. 표현식 사용시
	...
	<%
		...
		함수에서 JSP의 내장객체를 사용 할 때는 인자로 전달한 후
