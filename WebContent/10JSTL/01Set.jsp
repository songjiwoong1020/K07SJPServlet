...
<body>
	...
	set 태그 속성
		...
		scope : 영역을 지정. 문자열만 가능하고 4가지 영역을
			지정 할 수 있음. 기본값은 가장 좁은 page영역.
		즉, set태그를 사용하여 변수를 저장하면 지정된 영역에 저장된다.
	 -->
	<h2>set태그</h2>
	<h2>set태그로 EL에서 사용 할 변수 설정하기</h2>
	<ul>
		<li>
		1.value속성에 직접 값 대입 :
	...
	<h3>EL로 출력</h3>
	<!-- 
		EL로 영역의 속성을 읽어 올 때는 XXXScope를 생략하면 가장 좁은
		page영역을 읽어오게된다. 즉 아래는 모두 page영역을 읽어오게된다.
	 -->
	<ul>
	...
	<h3>인자생성자로 설정</h3>
	<!-- 인자생성자를 통해 DTO객체를 생성 한 후 request영역에 속성을 저장한다. -->
	<c:set var="argsMember" value='<%=new MemberDTO("KIM", "1111", "김태희", null) %>' scope="request"/>
	
	<!-- 영역에 저장될 때는 Object타입으로 자동형변환 되므로 반드시
	해당 자료형으로 변환(다운캐스팅)해야한다. -->
	<h3>JSP코드로 출력</h3>
	<ul>
	...
	않는다면 XXXScope 내장객체를 생략 할 수도 있다. -->
	<h3>EL로 출력</h3>
	<ul>
	...
	<!-- 
	Map계열의 컬렉션은 Key값을 통해 Value값에 접근한다. Key를 통한 접근이
	...
	<c:set target="${map.args }" property="name" value="으" />
