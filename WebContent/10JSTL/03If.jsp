...
<body>
	...
	if태그
		...
		-else문이 없기때문에 조건이 여러개일 때는 표현이 어렵다.
			->이 때는 주로 choose문을 사용한다.
		형식]	
			test : 조건
	...
	<!-- 
		if(numVar % 2 == 0)랑 동일한 조건으로, 해당조건은 true이므로
	...
	<%--
	<c:set var="u" value="${param.user }"/>
	<c:set var="p" value="${param.pass }"/>
	
	<c:if test="${u != null && p != null}">
		<c:if test="${u == 'kosmo' && p == '1234'}">
		코스모님 하이루
		</c:if>
	</c:if>	
	<c:if test="${u != null && p != null}">
