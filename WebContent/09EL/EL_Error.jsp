...
<body>
	...
	<fieldset>
		...
		<%-- 
			...
			에러가 발생하지는 않는다. => 0으로 출력되는 이유인듯
		 --%>
		<h2>특수문자가 포함된 속성에 .으로 접근시</h2>
		<ul>
			<li>\${header["user-agent"] } : ${header["user-agent"] } [정상]</li>
			<li>\${header.user-agent } : ${header.user-agent } [에러, 0 출력]</li>
		</ul>
		
		<h2>인덱스로 접근시(배열이나 리스트 계열 컬렉션)</h2>
		<%
			//문자열 배열 선언 후 List컬렉션에 각 요소값을 추가한다.
		...
		<%--
			배열이나 컬렉션처럼 인덱스로 접근하는 경우 .(닷)은 사용 할 수 없다.
		 --%>
		<ul>
		...
		<%--
			...
			배열이나 컬렉션의 크기만큼 반복하는 확장 for문의 형태로 기술됨.
		 --%>
		<h4>배열</h4>
		...
		<ul>
			...
			<%-- DTO객체에서 getId()를 주석 처리 후 테스트 해 볼것 --%>
			<li>속성은존재, Getter가 없는경우 : \${eldto.id } : \${eldto.id } [500에러발생]</li>
			<li>el변수명이 틀린경우 : \${noeldto.pwd } : ${noeldto.pwd } [에러NO, 미출력]</li>
		</ul>
		<h2>EL내장객체의 없는 속성으로 접근시</h2>
		<ul>
			<li>\${requestScope.nop } : ${requestScope.nop }
				[Scope계열 내장객체, 에러NO, 미출력 -> 속성명은 개발자 마음대로 정할 수 있기때문]</li>
			<%-- 
			EL을 통해 4가지 영역에 접근 할 때는 XXXScope패턴으로 사용한다.
			pageContext는 JSP에서 접근시 사용된다.
			 --%>
			<li>\${pageContext.nop } : \${pageContext.nop }
				[컴파일오류 발생 -> 내장객체명은 정해져 있기때문]</li>
		</ul>
		...
		<ul>
			...
			<li>\${requestScope.REQProperty } : ${requestScope.REQProperty } [에러NO, 미출력]</li>
